...
class TestFillDatabase(object):
    ...
    def test_fill_database(self, mock_configuration):
        ...
        for expected in data:
            db_object = krico.database.ClassifierInstance.filter(id=uuid.UUID(expected['id'])).allow_filtering().first()
            assert db_object.category == expected['category']
            assert db_object.name == expected['name']
            assert db_object.configuration_id == expected['host_aggregate']['configuration_id']
            assert db_object.parameters == expected['parameters']
            assert db_object.host_aggregate.disk == expected['host_aggregate']['disk']
            assert db_object.host_aggregate.ram == expected['host_aggregate']['ram']
            assert db_object.host_aggregate.name == expected['host_aggregate']['name']
            assert db_object.host_aggregate.configuration_id == expected['host_aggregate']['configuration_id']
            assert db_object.host_aggregate.cpu == expected['host_aggregate']['cpu']
            ...
            assert db_object.stop_time == datetime.datetime.strptime(expected['start_time'], "%Y-%m-%dT%H:%M:%S.%fZ")
            assert db_object.start_time == datetime.datetime.strptime(expected['start_time'], "%Y-%m-%dT%H:%M:%S.%fZ")

            db_object = krico.database.HostAggregate.filter(
                name=expected['host_aggregate']['name']).allow_filtering().first()
        ...
        for expected in data:
            db_object = krico.database.PredictorInstance.filter(id=uuid.UUID(expected['id'])).allow_filtering().first()
            assert db_object.instance_id == expected['instance_id']
            assert db_object.image == expected['image']
            assert db_object.category == expected['category']
            ...
            db_object = krico.database.Image.filter(image=expected['image']).allow_filtering().first()
            assert db_object.image == expected['image']
            assert db_object.category == expected['category']
